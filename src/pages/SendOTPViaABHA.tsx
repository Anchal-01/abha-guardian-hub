...
const SendOTPViaABHA = () => {
  ...
  const handleSendOtp = async () => {
    ...
    const endpoint =
      action === "delete"
        ? `${API_BASE}/curiomed/v1/abdm/delete/abha/byabha/request-otp`
        : `${API_BASE}/curiomed/v1/abdm/deactivate/abha/byabha/request-otp`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem(
          "abha-action-data",
          JSON.stringify({ abhaNumber, txnId: newTxnId, requestId: newRequestId, timestamp: newTimestamp })
        );
        toast({
          title: "OTP Sent",
  ...
  const handleVerifyOtp = async () => {
    ...
    const { txnId, requestId, timestamp } = data;

    setIsLoading(true);

    const endpoint =
      action === "delete"
        ? `${API_BASE}/curiomed/v1/abdm/delete/abha/byabha/verify-otp`
        : `${API_BASE}/curiomed/v1/abdm/deactivate/abha/byabha/verify-otp`;
